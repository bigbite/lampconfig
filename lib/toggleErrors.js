...
const toggleErrors = (status, phpVersion) => {
  shell.exec([
    `sudo sed -i "s/display_errors = .*/display_errors = ${capitalize(nextStatus)}/" /etc/php/${phpVersion}/fpm/php.ini`
    `sudo service php${phpVersion}-fpm restart > /dev/nul`
  ].join(' && '));
  console.log(chalk.green(`► PHP error logs turned ${nextStatus}.`));
};
...
module.exports = (store) => {
  ...
  store.get(['php_version', 'log_php_errors'], ([phpVersion, phpErrorStatus]) => {
    ...
    prompt(getAnswer).then((answer) => {
      ...
      toggleErrors(phpErrorStatus, nextStatus, phpVersion);
